$ ->
  ...
  $ window
    .on \scroll, ->
      if 0 < window.scroll-y < scroll-height
        set-parts-color ++parts-bgc
        set-morph-position!
        set-morph-form!
    .on \resize, ->
      square-side := Math.ceil window.inner-width / 10
    ...
    if window.scrollY < scroll-height / 2
      ...
      top =
        start:
          ...
          margin-top: szs.side / -2
          border-bottom-width: Math.ceil szs.wide-height
      ...
      morph =
        start:
          ...
          width: szs.side - 2   # хакота: по формулам считается или рендерится
          height: szs.side - 2  # неточно, поэтому приходится корректировать
          margin: szs.side / 2
