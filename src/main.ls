$ ->
  iscroll = new IScroll \.wrap, mouseWheel: true probeType: 3
  $morph = $ \.polymorph
  $top = $morph.find \.top
  ...
  $win = $ window
  last-top = -iscroll.y
  scroll-height = -iscroll.max-scroll-y
  ...
  # Events
  $win
    .on 'scroll touchmove', (e) -> e.preventDefault!
    .on \resize, ->
      iscroll.refresh!
      square-side := Math.ceil window.inner-width / 10
      scroll-height := -iscroll.max-scroll-y
  ...
  iscroll.on \scroll, ->
    set-parts-color ++parts-bgc if 0 < -iscroll.y < scroll-height
    set-morph-position!
    set-morph-form!
  ###
  ...
  function set-morph-position
    ...
    $morph.css \top, -iscroll.y / scroll-height * height
  ...
  function get-morph-styles settings
    ...
    for name, object of settings
      ...
      for style, value of object.start
        ...
        percent = -iscroll.y / scroll-height
